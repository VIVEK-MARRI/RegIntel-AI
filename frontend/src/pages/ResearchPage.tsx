  ...
  async function handleRun() {
    ...
    try {
      ...
      toast.push({
        ...
        description: (result.query || result.summary || "").slice(0, 80),
        tone: "success",
      });
    } catch (err) {
      toast.push({
      ...
      <div className="space-y-4">
        <Card padding="md">
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Run deep, multi-step regulatory research with a structured plan and grounded findings.
          </p>
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_120px_auto]">
            <Field label="Research question" id="research-query">
              <TextArea
                ...
                placeholder="e.g. Compare FEMA vs RBI reporting thresholds for FY26"
                rows={2}
              />
            </Field>
        ...
        {selected ? (
          ...
          <Card padding="none">
            <CardHeader
              ...
              description="Stored research artefacts"
            />
            <div className="card-body">
              {reports.isLoading ? (
                ...
                <EmptyState
                  ...
                  description="Run a research query above to generate your first report."
                />
              ) : (
                  ...
                  {reports.data.map((r) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm font-medium text-slate-900 dark:text-slate-100">
                        {truncate(r.summary || r.query, 140)}
                      </p>
      ...
      <div className="card-body space-y-5">
          ...
          <section>
            ...
            <ol className="mt-2 space-y-1.5">
              {report.plan.map((step, i) => (
                <li
                  key={step.step_id ?? `${i}`}
                  className="flex items-center gap-3 rounded-lg border border-slate-200 px-3 py-2 text-xs dark:border-slate-800"
                >
        ...
        {report.summary ? (
          <section>
            ...
            <p className="mt-2 whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-sm text-slate-700 dark:bg-slate-800/40 dark:text-slate-200">
              {report.summary}
            </p>
          </section>
        ) : null}
