...
export function CopilotPage() {
  ...
  const [activeConv, setActiveConv] = useState<string | undefined>(conversationId);
  ...
  useEffect(() => {
    if (messagesQuery.data?.items && activeConv) {
      setMessages(
    ...
    <div className="mx-auto grid h-full max-w-7xl grid-cols-1 gap-4 lg:grid-cols-[260px_minmax(0,1fr)_320px]">
      ...
      <Card padding="none" className="flex h-[calc(100vh-7rem)] flex-col">
        <CardHeader
          ...
          description={
            health.data
              ? `Connected · ${health.data.status} · ${health.data.module}`
              : "Connecting…"
          }
          actions={
        ...
        >
          {messages.length === 0 && !streaming ? (
            <EmptyState
              title="Ask the RegIntel Copilot"
              description="A regulatory copilot with citations, source attribution, and agent collaboration."
            />
          ) : (
        ...
        <div className="border-t border-slate-200 p-4 dark:border-slate-800">
          <div className="mb-2 flex flex-wrap gap-2">
            {SUGGESTED.map((s) => (
              <button
                ...
                type="button"
                onClick={() => setInput(s)}
                className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-medium text-slate-600 transition hover:border-brand-300 hover:text-brand-700
                           dark:border-slate-700 dark:bg-surface-dark-3 dark:text-slate-300 dark:hover:border-brand-500 dark:hover:text-brand-300"
              >
                {s}
              </button>
            ))}
          ...
          >
            <textarea
              className="input min-h-[64px] flex-1 resize-none"
              placeholder="Ask anything about regulations, compliance, or risk…"
              value={input}
...
function MessageBubble({ message }: { message: CopilotMessage }) {
  ...
  return (
    <li className={`flex items-start gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      <div
        className={`max-w-[85%] space-y-2 rounded-2xl border px-4 py-3 text-sm shadow-elevated ${
          isUser
      ...
      >
        ...
        {!isUser ? (
          <div className="space-y-3 pt-2">
            ...
            />
            <AgentContributionList contributions={message.agent_contributions ?? []} />
            <MemoryContextView ctx={message.memory_context} />
          </div>
        ) : null}
      </div>
    </li>
    ...
    <section>
      ...
      <ul className="mt-1.5 space-y-1.5">
        {citations.slice(0, 4).map((c, i) => (
          <li
            key={c.citation_id ?? `${i}`}
            className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] text-slate-700 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-200"
          >
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] text-slate-500 dark:text-slate-400">
                [{i + 1}] {c.source_label ?? c.document_id ?? c.chunk_id ?? c.citation_id}
              </span>
...
function SourceList({
  ...
  sources: import("@/types").CopilotAttribution[];
}) {
        ...
        {sources.slice(0, 6).map((s, i) => (
          <li key={s.source_id ?? `${i}`}>
            <Badge tone="neutral" size="sm">
              {s.document_title ?? s.document_id ?? s.source_id}
            </Badge>
          </li>
        ))}
      </ul>
  ...
  return (
    <section className="flex flex-wrap items-center gap-2 text-[11px]">
      ...
      {typeof faithfulness === "number" ? (
        <Badge tone={faithfulness > 0.75 ? "success" : "warning"}>
          Faithfulness {formatPercent(faithfulness)}
        </Badge>
      ) : null}
      {hallucinationDetected ? (
        <Badge tone="danger">Hallucination {hallucinationRisk ?? "detected"}</Badge>
      ) : null}
      {typeof latency === "number" ? (
        <Badge tone="neutral">{formatDurationMs(latency)}</Badge>
      ) : null}
    </section>
  );
}
...
function AgentContributionList({
  ...
  contributions: AgentContributionItem[];
}) {
    ...
    <section>
      ...
      <ul className="mt-1.5 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {contributions.map((c, i) => (
          <li
            key={`${c.agent_name}-${i}`}
            className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2 text-[11px] dark:border-slate-800 dark:bg-surface-dark-3"
          >
            <span className="flex h-5 w-5 items-center justify-center rounded bg-brand-500 text-[10px] font-bold text-white">
              {c.agent_name.slice(0, 1).toUpperCase()}
            </span>
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[10px] text-slate-500 dark:text-slate-400">
                {c.capability} · {formatDurationMs(c.duration_ms)}
              </p>
            </div>
            <Badge
              tone={c.status === "succeeded" ? "success" : c.status === "failed" ? "danger" : "warning"}
              size="sm"
            >
              {Math.round((c.confidence ?? 0) * 100)}%
            </Badge>
  ...
  return (
    <section>
      ...
      <div className="mt-1.5 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {ctx.short_term?.length ? (
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] dark:border-slate-800 dark:bg-slate-800/40">
            <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">
              Short-term
            </p>
            <ul className="mt-1 space-y-1">
              {ctx.short_term.slice(0, 3).map((m, i) => (
        ...
        {ctx.long_term?.length ? (
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] dark:border-slate-800 dark:bg-slate-800/40">
            <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">
              Long-term
            </p>
            <ul className="mt-1 space-y-1">
              {ctx.long_term.slice(0, 3).map((m, i) => (
                <li key={i} className="line-clamp-2 italic">
                  "{m.content}"
                </li>
              ))}
      ...
      {ctx.entities?.length ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {ctx.entities.slice(0, 6).map((e, i) => (
            <Badge key={i} tone="brand" size="sm">
              {e}
            </Badge>
          ))}
        </div>
      ...
      <Card padding="md" className="space-y-3">
        ...
        <ol className="space-y-2 text-xs text-slate-600 dark:text-slate-300">
          <li className="flex items-start gap-2">
            ...
            <span>Retrieves the most relevant regulatory chunks via hybrid search.</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500 text-[10px] font-bold text-white">
              2
            </span>
            <span>Routes through research, compliance, and risk agents.</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500 text-[10px] font-bold text-white">
              3
            </span>
            <span>Synthesises a citation-backed answer with confidence scores.</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500 text-[10px] font-bold text-white">
              4
            </span>
            <span>Logs decisions for governance and audit.</span>
          </li>
        </ol>
      </Card>
      <Card padding="md">
        ...
        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
          The copilot remembers your recent conversations and surfaces relevant
          long-term facts.
        </p>
      </Card>
    </>
