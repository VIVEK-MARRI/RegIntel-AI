...
async def test_retrieval_benchmarks(db_session):
    ...
    async def run_benchmark_set(name: str, metric: str, filters: dict, num_runs: int = 30):
        latencies = []
    ...
    results.append(await run_benchmark_set("Cosine Search (Doc Filter)", "cosine", {"document_id": doc.id}))
    ...
    print("| --- | --- | --- | --- | --- | --- |")
    for r in results:
        print(f"| {r['scenario']} | {r['metric']} | {r['avg_ms']:.2f} | {r['p95_ms']:.2f} | {r['p99_ms']:.2f} | {r['qps']:.1f} |")
    print("="*50 + "\n")
