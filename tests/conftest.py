...
if IS_SQLITE:
    ...
    TEST_DATABASE_URL = (
        f"postgresql+asyncpg://postgres:admin@localhost:5432/{TEST_DB_NAME}"
    )
    TEST_DATABASE_URL_SYNC = (
        f"postgresql+psycopg2://postgres:admin@localhost:5432/{TEST_DB_NAME}"
    )
...
def setup_test_db():
    ...
    conn = psycopg2.connect(
        host="localhost", dbname="postgres", user="postgres", password="admin"
    )
