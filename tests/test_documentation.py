...
EXPECTED_SECTIONS = {
    "01-system-architecture.md": ["## Overview", "## High-level diagram", "## Component responsibilities"],
    "02-agent-architecture.md": ["## Purpose", "## High-level diagram", "## Components"],
    "03-knowledge-graph.md": ["## Purpose", "## Data model", "## Storage", "## Performance"],
    "04-deployment-architecture.md": ["## Container topology", "## docker-compose", "## Security", "## Observability"],
    "05-data-flow.md": ["## 1. Ingest pipeline", "## 2. Search", "## 3. Agent run"],
    "06-components.md": ["## `app.main`", "## `app.api.v1.*`", "## `app.agent`", "## `app.security`", "## `app.benchmark`"],
    "07-api-reference.md": ["## Conventions", "## Core endpoints", "## Cross-Origin", "## Versioning"],
    "08-developer-guide.md": ["## Local setup", "## Testing", "## Code style"],
    "09-operations-guide.md": ["## Health probes", "## Metrics", "## Logs", "## Incident response"],
}
...
class TestFilePresence:
    ...
    def test_file_has_top_level_heading(self, filename: str) -> None:
        ...
        assert content.startswith("# "), f"{filename} does not start with a top-level heading"
...
class TestContentShape:
    ...
    def test_required_sections_present(self, filename: str, sections: list) -> None:
        ...
        for section in sections:
            assert section in content, f"{filename} missing required section {section!r}"
...
class TestCrossLinks:
    ...
    def test_every_file_links_to_readme(self) -> None:
        for filename in REQUIRED_FILES:
            ...
            assert "README" in content or "architecture" in content.lower(), (
...
class TestDiagramQuality:
    ...
    def test_data_flow_has_sequence_diagrams(self) -> None:
        ...
        assert content.count("sequenceDiagram") >= 3, (
    ...
    def test_deployment_has_topology_diagram(self) -> None:
        content = (ARCH_DIR / "04-deployment-architecture.md").read_text(encoding="utf-8")
        assert "flowchart" in content or "graph" in content
