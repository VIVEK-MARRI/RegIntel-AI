...
from app.services.structure.enricher import (
...
def test_chunk_metadata_builder():
    ...
    builder.set_document_id(doc_id).set_title("Test Doc").set_source("SEBI").set_page(
        5
    ).set_section("Section 1").set_subsection("Subsection 1.1").set_publication_date(
        date(2026, 5, 20)
    ).set_chunk_size(450).set_token_count(120).add_custom_metadata("custom_score", 0.95)
