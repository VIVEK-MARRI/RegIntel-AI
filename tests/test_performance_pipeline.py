...
class TestRequestContext:
    ...
    def test_request_context_to_log_dict(self):
        from app.services.observability import RequestContext
        ctx = RequestContext(endpoint="/search/hybrid", strategy="hybrid",
                             request_id="req-1")
        d = ctx.to_log_dict()
...
class TestAPIMetrics:
    ...
    def test_api_metrics_record_request(self):
        from app.services.observability import APIMetrics
        m = APIMetrics()
        m.record_request(endpoint="/search/dense", strategy="dense",
    ...
    def test_api_metrics_record_error(self):
        from app.services.observability import APIMetrics
        m = APIMetrics()
        m.record_request(endpoint="/search/bm25", strategy="bm25",
...
class TestHealthCheckSchemas:
    ...
    def test_health_checker_runs_checks(self):
        ...
        def _ok():
            ...
            return type("CH", (), {
                "name": "liveness", "status": HealthStatus.HEALTHY,
                "latency_ms": 0.5, "message": "ok", "details": {},
...
class TestPerformanceTracePatterns:
    ...
    def test_orchestrator_step_result_records_latency(self):
        ...
        sr = StepResult(step=PipelineStep.CITATION, status=PipelineStatus.SUCCESS,
                        latency_ms=45.2)
    ...
    def test_hybrid_search_response_has_latency(self):
        from app.schemas.hybrid_search import (
            HybridSearchResponse, HybridSearchDiagnostics,
        )
        from datetime import datetime, timezone
        diag = HybridSearchDiagnostics(
            query_type="text", query_confidence=0.9, recommended_strategy="hybrid",
            dense_count=5, bm25_count=5, fused_count=10, overlap_count=0,
            overlap_pct=0.0, dense_latency_ms=10.0, bm25_latency_ms=10.0,
            fusion_latency_ms=1.0, rerank_latency_ms=0.0, rerank_used=False,
            rerank_model=None, fusion_method="rrf",
        )
        resp = HybridSearchResponse(
            results=[], query="test", total_results=0,
            latency_ms=123.4, strategy="hybrid",
            request_id="r1", timestamp=datetime.now(timezone.utc),
            query_type="text", diagnostics=diag,
        )
    ...
    def test_analytics_service_records_retrieval_metrics(self):
        ...
        req = RetrievalMetricsCreate(
            query_id="q1", query_text="KYC", strategy="dense",
            retrieval_latency_ms=50.0, total_latency_ms=65.0,
        )
