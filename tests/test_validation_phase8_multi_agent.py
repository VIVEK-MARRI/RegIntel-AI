...
class TestIntelligenceAgentSchemas:
    ...
    def test_agent_collaboration_defaults(self):
        ...
        c = AgentCollaboration(from_agent="research", to_agent="compliance", request_kind="query")
...
class TestAuditAgentSchemas:
    ...
    def test_audit_agent_result_has_required_fields(self):
        ...
        r = AuditAgentResult(query="test", task_kind=AuditTaskKind.COMPLIANCE_VERIFICATION)
...
class TestOrchestrationPlatformSchemas:
    ...
    def test_orchestration_result_defaults(self):
        from app.schemas.orchestration import (
            OrchestrationResult, ExecutionMode,
            AgentExecutionGraph, AgentExecutionStep,
        )
        graph = AgentExecutionGraph(steps=[AgentExecutionStep(agent_name="a", capability="x")])
        r = OrchestrationResult(query="test", mode=ExecutionMode.SEQUENTIAL, execution_graph=graph)
    ...
    def test_workflow_definition_requires_name(self):
        from app.schemas.orchestration import WorkflowDefinition, AgentExecutionGraph, AgentExecutionStep
        g = AgentExecutionGraph(steps=[AgentExecutionStep(agent_name="x", capability="y")])
...
class TestResponseOrchestratorPipeline:
    ...
    def test_orchestrator_request_enforces_min_query(self):
        ...
        chunk = RetrievedChunk(chunk_id="c1", document_id="d1", content="test", score=0.5)
        with pytest.raises(ValidationError):
    ...
    def test_final_answer_response_constructs_with_all_required(self):
        ...
        answer = AnswerSection(executive_summary="test", detailed_explanation="test detail",
                               supporting_evidence=[], key_regulatory_references=[])
        citations = AnnotatedAnswer(executive_summary=AnnotatedText(text="test", citations=[]),
        ...
        resp = FinalAnswerResponse(
            ...
            confidence_score=0.5, confidence_level=ConfidenceLevel.MEDIUM,
            faithfulness_score=0.5, hallucination_detected=False,
...
class TestCrossOrchestratorIntegration:
    ...
    def test_orchestration_request_can_include_full_graph(self):
        from app.schemas.orchestration import (
            AgentExecutionGraph, AgentExecutionStep,
            OrchestrationRequest, ExecutionMode,
        )
        steps = [AgentExecutionStep(agent_name="a", capability="retrieval")]
        graph = AgentExecutionGraph(steps=steps)
        req = OrchestrationRequest(query="KYC", graph=graph, mode=ExecutionMode.PARALLEL)
    ...
    def test_shared_evidence_can_feed_into_citation(self):
        ...
        ev = SharedEvidenceItem(producer="research", kind="citation", title="evidence",
                                content={"text": "KYC required."})
        chunk = EvidenceChunk(
            chunk_id="ck1", document_id="d1",
            excerpt=ev.content["text"],
            page_number=1, section="sec",
        )
    ...
    def test_final_answer_response_compatible_with_citation_coverage(self):
        ...
        from app.schemas.confidence import ConfidenceLevel
        from app.schemas.hallucination import HallucinationRiskLevel
        answer = AnswerSection(executive_summary="test", detailed_explanation="test",
                               supporting_evidence=[], key_regulatory_references=[])
        citations = AnnotatedAnswer(executive_summary=AnnotatedText(text="test", citations=[]),
                                    detailed_explanation=AnnotatedText(text="test", citations=[]),
                                    supporting_evidence=[], key_regulatory_references=[],
        ...
        resp = FinalAnswerResponse(
            ...
            faithfulness_score=0.8, hallucination_detected=False,
            hallucination_risk_level=HallucinationRiskLevel.NONE,
            source_attributions=[], metadata=OrchestratorMetadata(),
        )
    ...
    def test_agent_result_can_be_aggregated_into_coordinator_result(self):
        ...
        r1 = AgentResult(agent_name="a", status=TaskStatus.SUCCEEDED, output={"summary": "ok"})
        r2 = AgentResult(agent_name="b", status=TaskStatus.SUCCEEDED, output={"summary": "ok"})
    ...
    def test_shared_evidence_store_cross_agent_lookup(self):
        ...
        store.add(SharedEvidenceItem(producer="research", kind="citation", title="c1", consumer="compliance"))
        store.add(SharedEvidenceItem(producer="compliance", kind="violation", title="v1", consumer="*"))
    ...
    def test_message_bus_cross_agent_communication(self):
        ...
        bus.publish(AgentMessage(from_agent="research", to_agent="compliance",
                                 kind=MessageKind.EVIDENCE, payload={"finding": "KYC gap"}))
...
class TestEdgeCases:
    ...
    def test_result_synthesizer_single_contribution(self):
        ...
        s = ResultSynthesizer()
        contrib = type("C", (), {
            "agent_name": "a", "status": "succeeded", "summary": "ok",
            "output": {"key": "val"}, "confidence": 0.8,
        })()
