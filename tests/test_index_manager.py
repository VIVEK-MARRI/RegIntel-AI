...
async def test_create_rebuild_drop_index(db_session):
    ...
    target_after = next((m for m in health_metrics_after if m["index_name"] == index_name), None)
    ...
    target_dropped = next((m for m in health_metrics_dropped if m["index_name"] == index_name), None)
