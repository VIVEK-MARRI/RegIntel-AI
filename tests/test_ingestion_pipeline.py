...
def create_test_pdf_bytes(content_lines: list[str], filename: str = "test.pdf") -> bytes:
...
@pytest.mark.asyncio
async def test_phase3_chunking_validation(client: AsyncClient, db_session: AsyncSession):
    ...
    # Verify chunks > 0
    assert len(chunks) > 0, f"Expected at least 1 chunk, got 0"
...
@pytest.mark.asyncio
async def test_phase4_embedding_validation(client: AsyncClient, db_session: AsyncSession):
