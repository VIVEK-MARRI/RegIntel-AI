...
async def test_bm25_benchmarks(db_session, tmp_path):
    ...
    async def run_benchmark_set(name: str, query: str, filters: dict, num_runs: int = 100):
        latencies = []
    ...
    results.append(await run_benchmark_set("BM25 Keyword Search (Unfiltered)", "KYC diligence", {}))
    
    # Scenario B: BM25 Query, Source Filter
    results.append(await run_benchmark_set("BM25 Keyword Search (Source Filter)", "KYC guidelines", {"source": SourceEnum.RBI}))
    
    # Scenario C: BM25 Query, Document Filter
    results.append(await run_benchmark_set("BM25 Keyword Search (Doc Filter)", "speed benchmark", {"document_id": doc.id}))
    
    # Scenario D: BM25 Query, Score Threshold Filter
    results.append(await run_benchmark_set("BM25 Keyword Search (Threshold Filter)", "KYC diligence", {"score_threshold": 1.0}))
    ...
    print("| --- | --- | --- | --- | --- | --- |")
    for r in results:
        print(f"| {r['scenario']} | {r['query']} | {r['avg_ms']:.2f} | {r['p95_ms']:.2f} | {r['p99_ms']:.2f} | {r['qps']:.1f} |")
    print("="*50 + "\n")
