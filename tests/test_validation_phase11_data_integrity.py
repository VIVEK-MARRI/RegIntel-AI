...
class TestAuditEngineHashChain:
    ...
    def test_audit_record_has_hash_fields(self):
        ...
        r = AuditRecord(
            audit_id="a1", actor="system", action=AuditAction.CREATE,
    ...
    def test_audit_engine_append_creates_chain(self):
        ...
        r1 = svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.CREATE,
            subject_type="document", subject_id="d1",
        ))
        r2 = svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.UPDATE,
            subject_type="document", subject_id="d1",
        ))
    ...
    def test_audit_engine_verify_valid_chain(self):
        ...
        store = InMemoryAuditStore()
        svc = AuditService(store)
        svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.CREATE,
            subject_type="document", subject_id="d1",
        ))
        svc.create_record(AuditRecordCreateRequest(
...
class TestAuditStats:
    ...
    def test_audit_stats_chain_length(self):
        ...
        svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.CREATE,
            subject_type="doc", subject_id="d1",
        ))
        svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.UPDATE,
            subject_type="doc", subject_id="d1",
...
class TestAuditLog:
    ...
    def test_audit_log_entry_has_duration(self):
        ...
        log.record(AuditLogEntry(
            timestamp=datetime.now(timezone.utc),
            request_id="req1",
            method="GET", path="/health", status_code=200, duration_ms=12.3,
        ))
        entry = log.all()[0]
    ...
    def test_audit_log_all_returns_all_entries(self):
        ...
        log.record(AuditLogEntry(
            timestamp=datetime.now(timezone.utc),
            request_id="req1",
            method="GET", path="/health", status_code=200, duration_ms=5.0,
        ))
        log.record(AuditLogEntry(
            timestamp=datetime.now(timezone.utc),
            request_id="req2",
            method="POST", path="/api/v1/documents", status_code=201, duration_ms=50.0,
        ))
        entries = log.all()
...
class TestIngestionAuditTrail:
    ...
    def test_ingestion_audit_entry_has_required_fields(self):
        ...
        entry = IngestionAuditEntry(
            run_id="r1", step="download", event="started",
        )
    ...
    def test_ingestion_audit_entry_defaults(self):
        from app.schemas.ingestion import IngestionAuditEntry
        entry = IngestionAuditEntry(
            run_id="r1", step="chunk", event="completed",
    ...
    def test_ingestion_audit_entry_timestamps(self):
        from app.schemas.ingestion import IngestionAuditEntry
        entry = IngestionAuditEntry(
            run_id="r1", step="embed", event="started",
        )
    ...
    def test_ingestion_audit_entry_severity_levels(self):
        from app.schemas.ingestion import IngestionAuditEntry
        entry = IngestionAuditEntry(
            run_id="r1", step="index", event="failed",
            level="error",
        )
...
class TestDocumentIntegrity:
    ...
    def test_document_checksum_length_64(self):
        ...
        d = DocumentCreate(
            title="test", source=SourceEnum.RBI,
            file_name="t.pdf", file_path="/tmp/t.pdf",
            checksum="a" * 64,
        )
...
class TestGovernanceTracking:
    ...
    def test_audit_trail_manager_lineage(self):
        ...
        svc.create_record(AuditRecordCreateRequest(
            actor="system", action=AuditAction.APPROVE,
            subject_type="document", subject_id="d1",
        ))
