...
class MockEmbeddingProvider:
    ...
    def get_model_name(self) -> str: return "phase6-mock"
    def get_dimension(self) -> int: return 3
    def encode_query(self, query: str) -> List[float]:
        q = query.lower()
        if "kyc" in q: return [1.0, 0.0, 0.0]
        if "aml" in q: return [0.8, 0.2, 0.0]
        if "mutual" in q or "fund" in q: return [0.0, 1.0, 0.0]
        if "rbi" in q or "master circular" in q: return [0.5, 0.3, 0.2]
        if "sebi" in q: return [0.2, 0.5, 0.3]
        return [0.0, 0.0, 1.0]
...
class MockReranker:
    ...
    def rerank(self, query, candidates, *, top_k=None, score_threshold=None):
        from app.schemas.reranker import RerankResult, RerankResponse, RerankReport, PrecisionMetrics, ScoreDistribution
...
async def seeded_corpus(db_session: AsyncSession) -> Dict[str, Any]:
    ...
    chunks = [
        DocumentChunk(document_id=doc_rbi.id, page_number=1, section="KYC",
        ...
        DocumentChunk(document_id=doc_sebi.id, page_number=2, section="Insider Trading",
            ...
            token_count=20),
        DocumentChunk(document_id=doc_irda.id, page_number=1, section="AML",
...
class TestDenseRetrieval:
    ...
    async def test_dense_search_exact_query(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={
            "query": "KYC verification", "top_k": 5
        })
    ...
    async def test_dense_search_paraphrased_query(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={
            "query": "customer identification documents", "top_k": 5
        })
    ...
    async def test_dense_search_synonym_query(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={
            "query": "client verification", "top_k": 5
        })
        assert resp.status_code == 200
        body = resp.json()
    ...
    async def test_dense_search_semantic_only(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={
            "query": "financial compliance regulations", "top_k": 5
        })
        assert resp.status_code == 200
        body = resp.json()
...
class TestBM25Retrieval:
    ...
    async def test_bm25_keyword_search(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/bm25", json={
            "query": "Aadhaar PAN", "top_k": 5
        })
        assert resp.status_code == 200, resp.text
        body = resp.json()
    ...
    async def test_bm25_exact_regulatory_references(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/bm25", json={
            "query": "Suspicious Transaction Reports", "top_k": 5
        })
        assert resp.status_code == 200
        body = resp.json()
    ...
    async def test_bm25_section_numbers(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/bm25", json={
            "query": "Mutual Funds section", "top_k": 5
        })
        assert resp.status_code == 200
        body = resp.json()
        assert body["total_results"] > 0
    ...
    async def test_bm25_exact_phrase(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/bm25", json={
            "query": "\"unpublished price sensitive information\"", "top_k": 5
        })
        assert resp.status_code == 200
        body = resp.json()
        assert body["total_results"] > 0
...
class TestHybridRetrieval:
    ...
    async def test_hybrid_search_returns_results(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/hybrid", json={
            "query": "KYC compliance", "top_k": 5,
...
class TestRRFFusion:
    ...
    def test_rrf_fusion_ranks_overlap_higher(self):
        ...
        fused = engine.fuse_results(dense_results, bm25_results, method=FusionMethod.RRF)
    ...
    def test_rrf_overlap_result_boosted(self):
        ...
        engine = FusionEngine()
        dense = [{"chunk_id": str(i), "score": float(100 - i), "content": "", "metadata": {}} for i in range(5)]
        bm25 = [{"chunk_id": str(i), "score": float(100 - i), "content": "", "metadata": {}} for i in range(5)]
    ...
    def test_rrf_score_normalization(self):
        ...
        engine = FusionEngine()
        dense = [{"chunk_id": str(i), "score": float(i * 10), "content": "", "metadata": {}} for i in range(3)]
        bm25 = [{"chunk_id": str(i + 1), "score": float(i * 5), "content": "", "metadata": {}} for i in range(3)]
        fused = engine.fuse_results(dense, bm25, method=FusionMethod.RRF)
        for r in fused:
            score = r.get("score", 0)
...
class TestBGEReranker:
    ...
    def test_reranker_changes_order(self):
        ...
        assert result.results[0].chunk_id == "a"  # mock gives a the highest rerank_score
    ...
    def test_reranker_relevant_chunks_move_up(self):
        ...
        reranker = MockReranker()
        candidates = [
            {"chunk_id": "low", "score": 0.3, "content": "low relevance", "metadata": {}},
            {"chunk_id": "high", "score": 0.8, "content": "high relevance", "metadata": {}},
        ]
    ...
    def test_reranker_top_k_quality(self):
        ...
        reranker = MockReranker()
        candidates = [
            {"chunk_id": f"c{i}", "score": 1.0 - 0.1 * i, "content": f"content {i}", "metadata": {}}
            for i in range(10)
        ]
...
class TestKGExpansion:
    ...
    async def setup_kg(self):
        ...
        svc.add_relationship(
            RelationshipCreateRequest(
                source_id=reg.node_id, target_id=circ.node_id,
                relationship_type=RelationshipType.AMENDS,
            )
        )
        svc.add_relationship(
            RelationshipCreateRequest(
                source_id=circ.node_id, target_id=topic.node_id,
...
class TestEdgeCases:
    ...
    async def test_single_word_query(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={"query": "KYC", "top_k": 5})
        assert resp.status_code == 200
    ...
    async def test_no_result_query(self, client, seeded_corpus):
        ...
        resp = await client.post("/api/v1/search/dense", json={
            "query": "ZZZZZZZZZZnotexist", "top_k": 5
        })
        assert resp.status_code == 200
...
class TestPerformance:
    ...
    async def test_bm25_latency(self, client, seeded_corpus):
        ...
        for _ in range(5):
            resp = await client.post("/api/v1/search/bm25", json={
                "query": "Aadhaar PAN KYC", "top_k": 10
            })
            assert resp.status_code == 200
        elapsed = (time.time() - start) / 5
    ...
    async def test_scores_are_meaningful(self, client, seeded_corpus):
        ...
        dense_resp = await client.post("/api/v1/search/dense", json={
            "query": "KYC", "top_k": 3
        })
        bm25_resp = await client.post("/api/v1/search/bm25", json={
            "query": "KYC", "top_k": 3
        })
