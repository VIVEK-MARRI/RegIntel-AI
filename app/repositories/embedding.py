...
class ChunkEmbeddingRepository(BaseRepository[ChunkEmbedding]):
    ...
    async def get_embedding(
        self, chunk_id: uuid.UUID, embedding_model: str
    ...
    async def save_embeddings_bulk(self, embeddings_data: list[dict]) -> None:
        ...
        stmt = stmt.on_conflict_do_update(
            index_elements=["chunk_id", "embedding_model"], set_=update_dict
        )
        await self.db_session.execute(stmt)
        await self.db_session.flush()

    async def delete_embeddings(
        self, chunk_id: uuid.UUID, embedding_model: Optional[str] = None
    ) -> None:
    ...
    async def get_document_embeddings_status(
