...
async def semantic_search(
    ...
    try:
        ...
        effective_top_k = (
            request.skip + request.limit if request.skip > 0 else request.top_k
        )
        ...
        return SearchResponse(
            query=request.query, results=items, trace=search_res.get("trace")
        )
...
async def get_embedding_stats(
    ...
    try:
        ...
        emb_stmt = select(func.count(ChunkEmbedding.id)).where(
    ...
    try:
        ...
        if request.index_name:
            await index_manager.rebuild_index(
                index_name=request.index_name, concurrently=request.concurrently
            )
            ...
            if not matched_indexes:
                ...
                for idx_name in matched_indexes:
                    await index_manager.rebuild_index(
                        index_name=idx_name, concurrently=request.concurrently
                    )
                    rebuilt_indexes.append(idx_name)

        return IndexRebuildResponse(status="success", rebuilt_indexes=rebuilt_indexes)
    except Exception as e:
