...
async def list_chunks(
    document_id: Optional[uuid.UUID] = Query(
        None, description="Filter chunks by document ID"
    ),
    section: Optional[str] = Query(
        None, description="Search by section (partial match)"
    ),
    subsection: Optional[str] = Query(
        None, description="Search by subsection (partial match)"
    ),
    sort_by: ChunkSortByEnum = Query(
        ChunkSortByEnum.page_number, description="Field to sort by"
    ),
    sort_order: SortOrderEnum = Query(
        SortOrderEnum.asc, description="Sort direction (asc or desc)"
    ),
