...
class BaseValidationRule(ABC):
    ...
    def validate_chunk(self, chunk: Dict[str, Any], context: Dict[str, Any] = None) -> Optional[ValidationIssue]:
        """Validates a single chunk. Returns a ValidationIssue if invalid, else None."""
        return None

    def validate_batch(self, chunks: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[ValidationIssue]:
...
class MissingSectionRule(BaseValidationRule):
    ...
    def __init__(self, invalid_sections: Optional[Set[str]] = None):
        self.invalid_sections = invalid_sections if invalid_sections is not None else {"General"}
...
class MalformedHierarchyRule(BaseValidationRule):
    ...
    def validate_chunk(self, chunk: Dict[str, Any], context: Dict[str, Any] = None) -> Optional[ValidationIssue]:
        ...
        if subsection and str(subsection).strip():
            if not section or not str(section).strip() or str(section).strip() == "General":
                return ValidationIssue(
                    chunk_id=str_chunk_id,
                    rule_name=self.name,
