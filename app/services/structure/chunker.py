...
class HierarchicalChunker:
    ...
    def chunk_document(
        self, document_id: uuid.UUID, doc_title: str, pages: List[Dict[str, Any]]
    ) -> List[ChunkResponse]:
        ...
        if not segments and current_segment_lines:
            segments.append(
                {"section": "General", "subsection": "", "lines": current_segment_lines}
            )
        ...
        for seg in segments:
            ...
            seg_chunks = self._chunk_segment_lines(
                document_id, doc_title, sec, sub, lines
            )
    ...
    def _chunk_segment_lines(
        ...
        while i < len(lines):
            ...
            if current_tokens + line_tokens > 800 and current_tokens >= 500:
                ...
                chunk_page = (
                    current_chunk_lines[0][1] if current_chunk_lines else page_num
                )
                ...
                chunk_uuid = uuid.uuid5(
                    ...
                    f"{section}:{subsection}:{content_hash}",
                )
        ...
        if current_chunk_lines:
            ...
            content_hash = hashlib.sha256(chunk_content.encode("utf-8")).hexdigest()
            chunk_uuid = uuid.uuid5(
                uuid.UUID(str(document_id)), f"{section}:{subsection}:{content_hash}"
            )
...
class HierarchicalChunkerService:
    ...
    async def chunk_document_by_id(
        ...
        for p in pages_data:
            logger.info(
                "Page %d: content length=%d, preview=%s",
                p["page_number"],
                len(p["content"]),
                repr(p["content"][:100]),
            )
