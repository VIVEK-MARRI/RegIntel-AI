...
class ParserService:
    ...
    async def parse_document(
        ...
        if not file_path.exists() or not file_path.is_file():
            ...
            await self.document_service.update_document_status(doc.id, StatusEnum.FAILED)
        ...
        try:
            ...
            if doc.page_count != total_pages:
                ...
                await self.document_service.update_document_metadata(
                    doc.id, 
                    DocumentUpdate(page_count=total_pages)
                )
            ...
            for i in range(total_pages):
                ...
                logger.debug(f"Parsed page {page_num}/{total_pages} for document {doc.id}")
