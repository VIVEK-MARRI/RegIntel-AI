...
class RetrievalBenchmarkRunner:
    ...
    def __init__(
        self, retrieval_service: RetrievalService, history_dir: Optional[str] = None
    ):
        self.retrieval_service = retrieval_service
        self.history_dir = history_dir or os.path.join(
            settings.STORAGE_ROOT, "benchmarks", "history"
        )
    ...
    async def run_benchmark(
        ...
        for item in golden_dataset:
            ...
            response = await self.retrieval_service.retrieve(
                query=item.query, top_k=top_k, distance_metric=distance_metric
            )
            ...
            expected_count = max(
                len(item.expected_chunk_ids), len(item.expected_sections), 1
            )
        ...
        hit_rate10 = (
            sum(1.0 if q.hit_at_10 else 0.0 for q in query_results) / num_queries
        )
        ...
        model_safe = "".join(c if c.isalnum() else "_" for c in report.embedding_model)
        filename = (
            f"{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{model_safe}.json"
        )
