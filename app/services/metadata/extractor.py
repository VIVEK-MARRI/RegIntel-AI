...
class FileMetadataExtractor(MetadataExtractor):
    ...
    def extract(
        ...
        return {"file_size": file_size, "checksum": checksum}
    ...
    def extract(
        ...
        try:
            ...
            if "reserve bank of india" in first_page_text.lower():
                ...
                "securities and exchange board of india" in first_page_text.lower()
                or "sebi" in first_page_text.lower()
            ):
            ...
            rbi_circ_match = re.search(
                r"(RBI/\d{4}-\d{2}/\d+|RBI/[A-Z0-9/-]+/\d{4}-\d{2}/\d+)",
                first_page_text,
            )
            # SEBI pattern e.g., SEBI/HO/CFD/CMD1/CIR/P/2026/123 or SEBI/LAD-NRO/GN/2026/89
            sebi_circ_match = re.search(
                r"(SEBI/[A-Z0-9/-]+/CIR/[A-Z0-9/-]+/\d{4}/\d+|SEBI/LAD-NRO/[A-Z0-9/-]+/\d{4}/\d+)",
                first_page_text,
            )
            ...
            reg_match = re.search(
                r"(Notification\s+No\.\s+[A-Za-z0-9-/]+|No\.\s*[A-Za-z0-9-/]+)",
                first_page_text,
                re.IGNORECASE,
            )
            if reg_match:
            ...
            logger.warning(
                f"RegulatorMetadataExtractor failed to parse {file_path}: {e}"
            )
            return {}
