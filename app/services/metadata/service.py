...
class MetadataService:
    ...
    def __init__(self, extractors: List[MetadataExtractor] = None):
        ...
        logger.info(
            f"Initialized MetadataService with {len(self.extractors)} extractors"
        )
    ...
    def register_extractor(self, extractor: MetadataExtractor) -> None:
        ...
        logger.info(
            f"Registered new metadata extractor: {extractor.__class__.__name__}"
        )

    def extract_metadata(
        self, file_path: Path, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        merged_metadata = {"upload_timestamp": datetime.now(timezone.utc).isoformat()}
