...
class EmbeddingValidationIssue(BaseModel):
    ...
    chunk_id: Optional[str] = Field(
        None, description="The ID of the chunk associated with the embedding."
    )
    embedding_id: Optional[str] = Field(
        None, description="The primary key ID of the ChunkEmbedding record."
    )
...
class EmbeddingValidationMetrics(BaseModel):
    ...
    embedding_coverage: float = Field(
        ..., description="Percentage of chunks with valid COMPLETED embeddings."
    )
    average_vector_norm: float = Field(
        ..., description="Average L2 norm of valid embedding vectors."
    )
    invalid_embedding_count: int = Field(
        ..., description="Total count of invalid embedding records."
    )
    duplicate_embedding_count: int = Field(
        ..., description="Total count of duplicate embeddings."
    )
...
class EmbeddingValidationReport(BaseModel):
    ...
    valid: bool = Field(
        ..., description="True if no ERROR issues were found, False otherwise."
    )
    issues: List[EmbeddingValidationIssue] = Field(
        default_factory=list, description="List of detected validation issues."
    )
    metrics: EmbeddingValidationMetrics = Field(
        ..., description="Calculated embedding metrics."
    )
