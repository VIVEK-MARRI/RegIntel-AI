...
class QueryAnalysisResponse(BaseModel):
    ...
    query_type: str = Field(
        ...,
        description="Classified query type (keyword, circular, regulation, semantic, comparison, definition).",
    )
...
class HybridRerankSearchRequest(BaseModel):
    ...
    top_k: int = Field(
        5, ge=1, le=100, description="Final number of results after reranking."
    )
    rerank_top_k: Optional[int] = Field(
        None,
        ge=1,
        le=100,
        description="Candidates to pass to reranker (defaults to fusion_candidate_k).",
    )
    rerank_score_threshold: float = Field(
        0.0, ge=0.0, le=1.0, description="Minimum reranker score to include."
    )
    fusion_candidate_k: int = Field(
        20, ge=1, le=200, description="Number of fused candidates before reranking."
    )
