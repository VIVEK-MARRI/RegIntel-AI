...
class ScoreDistribution(BaseModel):
    ...
    bins: List[float] = Field(default_factory=list, description="Score bin edges [0.0, 0.1, 0.2, ...].")
    counts: List[int] = Field(default_factory=list, description="Count of scores falling in each bin.")
    median: Optional[float] = Field(None, description="Median reranker score.")
    std_dev: Optional[float] = Field(None, description="Standard deviation of reranker scores.")
...
class PrecisionMetrics(BaseModel):
    ...
    rank_correlation: Optional[float] = Field(
        None, description="Spearman rank correlation between original and rerank scores."
    )
...
class RerankReport(BaseModel):
    ...
    candidates_filtered: int = Field(0, description="Number of candidates removed by score threshold.")
    latency_ms: float = Field(0.0, description="Total wall-clock reranking latency in milliseconds.")
    scoring_latency_ms: float = Field(0.0, description="Time spent in model inference only.")
...
class BenchmarkResult(BaseModel):
    ...
    scoring_latency_ms: float = Field(..., description="Model inference latency in milliseconds.")
...
class BenchmarkReport(BaseModel):
    ...
    total_candidates: int = Field(0, description="Total number of candidates processed.")
    avg_latency_ms: float = Field(0.0, description="Average total latency per query in milliseconds.")
    p50_latency_ms: float = Field(0.0, description="50th percentile latency in milliseconds.")
    p95_latency_ms: float = Field(0.0, description="95th percentile latency in milliseconds.")
    p99_latency_ms: float = Field(0.0, description="99th percentile latency in milliseconds.")
    avg_scoring_latency_ms: float = Field(0.0, description="Average model inference latency.")
    throughput_qps: float = Field(0.0, description="Queries per second throughput.")
    avg_candidates_per_query: float = Field(0.0, description="Average number of candidates per query.")
    avg_top_score: float = Field(0.0, description="Average top reranker score across all queries.")
