...
class FusionConfig(BaseModel):
    ...
    method: FusionMethod = Field(
        FusionMethod.RRF, description="Fusion algorithm to apply."
    )
    rrf_k: int = Field(
        60, ge=1, description="Constant for the RRF formula: 1/(k + rank)."
    )
    dense_weight: float = Field(
        0.5, ge=0.0, le=1.0, description="Weight for dense retrieval scores."
    )
    bm25_weight: float = Field(
        0.5, ge=0.0, le=1.0, description="Weight for BM25 retrieval scores."
    )
...
class FusedCandidate(BaseModel):
    ...
    rrf_score: Optional[float] = Field(
        None, description="RRF-specific score (when RRF is used)."
    )
    dense_score: Optional[float] = Field(
        None, description="Original dense similarity score."
    )
    bm25_score: Optional[float] = Field(
        None, description="Original BM25 relevance score."
    )
    dense_rank: Optional[int] = Field(
        None, description="1-indexed rank in the dense result list."
    )
    bm25_rank: Optional[int] = Field(
        None, description="1-indexed rank in the BM25 result list."
    )
    sources: List[str] = Field(
        default_factory=list,
        description="Retrieval sources that contributed this chunk (e.g. ['dense', 'bm25']).",
    )
...
class FusionReport(BaseModel):
    ...
    dense_count: int = Field(
        0, description="Number of candidates from dense retrieval."
    )
    ...
    overlap_count: int = Field(
        0, description="Number of chunks that appeared in both lists."
    )
    overlap_percentage: float = Field(
        0.0, description="Overlap as a percentage of the union."
    )
    config: FusionConfig = Field(
        default_factory=FusionConfig,
        description="The configuration used for this fusion run.",
    )
